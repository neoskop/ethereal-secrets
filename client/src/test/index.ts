...
import {
  EtherealSecretsClient, EtherealSecretsClientConfig, RemoteRetrieveResult,
  RemoteSaveResult
} from "../index";
...
function createClient(config: Object = {}): EtherealSecretsClient {
  let defaultConfig: EtherealSecretsClientConfig = {endpoint: 'http://server:8080'};
...
describe('Ethereal Secrets Client', () => {
  ...
  it('should save to local storage if requested', () => {
    let sut: EtherealSecretsClient = createClient({storage: localStorage});
    return sut.saveLocal('foo', 'bar').should.eventually.be.fulfilled.and.then(() => {
  ...
  it('should return saved item unchanged', () => {
    let sut: EtherealSecretsClient = createClient();
    return sut.saveLocal('foo', 'bar').should.eventually.be.fulfilled.and.then(() => {
      return sut.getLocal('foo').should.eventually.be.fulfilled.and.then(value => value.should.equal('bar'));
    });
  });
  ...
  it('should allow saving arbitrary data remotely and return a key to it', () => {
    let sut: EtherealSecretsClient = createClient();
    return sut.saveRemote('foo').should.eventually.be.fulfilled.and.then((result: RemoteSaveResult) => {
      return Promise.all([
  ...
  it('should return input data unchanged when using returned fragment identifier', () => {
    let sut: EtherealSecretsClient = createClient();
    return sut.saveRemote('foo').should.eventually.be.fulfilled.and.then((result: RemoteSaveResult) => {
      return sut.getRemote(result.fragmentIdentifier).should.eventually.be.fulfilled.and.then((result: RemoteRetrieveResult) => {
